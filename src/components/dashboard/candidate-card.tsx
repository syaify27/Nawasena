    ...
    <Card className="flex flex-col overflow-hidden transition-all hover:shadow-lg">
      <CardHeader className="flex-row items-start gap-4 space-y-0 bg-muted/20">
        <Avatar className="h-12 w-12 border">
          <AvatarImage src={employee.avatar} alt={employee.nama} data-ai-hint="person portrait" />
        ...
        <Badge variant="secondary" className="text-lg font-bold text-primary">#{rank}</Badge>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col justify-between p-4">
        <div>
          ...
          <Progress value={score} className="h-3 [&>div]:" style={{ '--tw-bg-opacity': 1, backgroundColor: getScoreColor(score) } as React.CSSProperties} />
        </div>
      </CardContent>
